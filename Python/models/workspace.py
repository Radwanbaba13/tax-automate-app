...
def createCoupleWordDocMultiYear(couple_summaries, output_file_path):
...
def createCoupleWordDocEN(couple_summaries, output_file_path):
    ...
    isMailQC = any(ind['isMailQC'] for ind in couple_summaries)
    isNewcomer = any(ind['isNewcomer'] for ind in couple_summaries)

    primary_individual = next(ind for ind in couple_summaries if ind['isPrimary'])
    secondary_individual = next(ind for ind in couple_summaries if not ind['isPrimary'])

    # Get all years sorted
    all_years = sorted(set(primary_individual['summaries'].keys()) | set(secondary_individual['summaries'].keys()))

    # Add section_1 with formatted years
    section_1(doc, primary_individual['summaries'][all_years[0]], primary_individual['title'], all_years, all_years[-1], isMailQC, couple=True,
              secondary_summary=secondary_individual['summaries'].get(all_years[0], {}), 
              secondary_ind_title=secondary_individual['title'])

    for year in all_years:
        primary_summary = primary_individual['summaries'].get(year, {})
        secondary_summary = secondary_individual['summaries'].get(year, {})

        para = doc.add_paragraph()
        run = para.add_run(f"{year}")
        run.underline = True

        tax_return(doc, primary_summary, secondary_summary, primary_individual['title'], secondary_individual['title'], isCouple=True, year=year)

        for individual in couple_summaries:
            return_summary = individual['summaries'].get(year, {})
            if "gst_amounts" in return_summary and return_summary["gst_amounts"]:
                gst_credit(doc, return_summary, isNewcomer, year, all_years[-1], all_years[0])

            if "ecgeb_amounts" in return_summary and return_summary["ecgeb_amounts"]:
                ecgeb_credit(doc, return_summary, isNewcomer, year, all_years[-1], all_years[0])

            if "solidarity_amounts" in return_summary and return_summary["solidarity_amounts"]:
                solidarity_credit(doc, return_summary, year, all_years[-1])

            if "ccb_amounts" in return_summary and return_summary["ccb_amounts"]:
                child_benefit(doc, return_summary, year, all_years[-1])

            if "family_allowance_amounts" in return_summary and return_summary["family_allowance_amounts"]:
                quebec_family_allowance(doc, return_summary, year, all_years[-1])
...
def section_1(doc, primary_summary, ind_title, years, isMailQC, couple=False, secondary_summary=None, secondary_ind_title=None):
    ...
    para = doc.add_paragraph()
    para.add_run(f'We have attached all the documents related to your {formatted_years} tax declarations. ').bold = False
    para.add_run('The password on all files consists of the nine digits of your Social Insurance Number.\n').bold = True
    para.add_run(f'The documents named COPY are copies of your complete tax returns. ').bold = False
    para.add_run('You do not need to print them or sign them; ').bold = True
    para.add_run('please keep them for your record. Please review them carefully to make sure they are accurate and complete.').bold = False
...
def tax_return(doc, return_summary, year, last_year, secondary_summary=None, primary_title=None, secondary_title=None, isCouple=False):
    ...
    if year == last_year:
        add_payment_section(doc, federal_owing, quebec_owing, year, last_year)
...
def add_payment_section(doc, federal_owing, quebec_owing, year, last_year):
    if federal_owing > 0 or quebec_owing > 0:
        ...
        if last_year == year:
            para.add_run(f'please make sure to pay the balance due by April 30, {int(year) + 1}, to avoid paying any interest. ')
       
        para.add_run('Please wait a few days after we E-file to pay your outstanding balance. For more details on how to pay the amount due, please click on: ')
        ...
        if len(filtered_months) <= 4:
            ...
            if len(unique_amounts) == 1:
                ...
                for start, end, amount, year in amount_ranges:
                    para.add_run(f'${amount:,.2f}/month from {start} to {end} {year}')
...
def createCoupleWordDocFR(couple_summaries, output_file_path):
    ...
    conclusionFR(doc, isCouple=True)
...
def section_1FR(doc, return_summary, ind_title, year, isMailQC, couple=False, secondary_summary=None, secondary_ind_title=None):
    ...
    para.add_run('Le mot de passe sur tous les fichiers se compose des neuf chiffres de votre numéro d’assurance sociale.\n').bold = True
    if couple:
        para.add_run('Les documents nommés COPIE sont des copies complètes de vos déclarations de revenus. ').bold = False
    ...
    if isMailQC:
        ...
        federal_title_run.bold = True
        federal_title_run.underline = True
        para.add_run('Noter que votre déclaration d’impôt Fédérale ').bold = True
...
def tax_returnFR(doc, return_summary, year, secondary_summary=None, primary_title=None, secondary_title=None, isCouple=False):
    ...
    def add_payment_section(para, federal_owing, quebec_owing):
        if federal_owing > 0 or quebec_owing > 0:
            para = doc.add_paragraph()
            ...
            if federal_owing > 0:
                add_hyperlink(para, 'Fédéral', 'https://www.canada.ca/fr/agence-revenu/services/paiements/payer-cra.html')
            if federal_owing > 0 and quebec_owing > 0:
                para.add_run(' et ')
            if quebec_owing > 0:
                add_hyperlink(para, 'Québec', 'https://www.revenuquebec.ca/fr/citoyens/declaration-de-revenus/paiement-dun-solde-dû-ou-reception-dun-remboursement/solde-dimpot-dû/')
    ...
    add_payment_section(para, federal_owing, quebec_owing)
...
def carryforward_amountsFR(doc, return_summary):
    ...
    if isinstance(federal_tuition_amount, str):
        federal_tuition_amount = float(federal_tuition_amount)
    if isinstance(quebec_tuition_8_percent, str):
        quebec_tuition_8_percent = float(quebec_tuition_8_percent)

    # Format the amounts without decimals
...
def child_benefitFR(doc, return_summary, year):
    ...
    if len(filtered_months) <= 4:
        ...
        if len(unique_amounts) == 1:
            ...
            for start, end, amount, year in amount_ranges:
                para.add_run(f'{format_currency(amount)} /mois de {start} à {end} {year}\n')
...
def conclusionFR(doc, isCouple=False):
    ...
    # Add the red text above the conclusion
    if isCouple:
        red_run = para.add_run('\nNous attendons les formulaires d’autorisation signés pour soumettre vos déclarations.\n')
    else:
        red_run = para.add_run('\nNous attendons le formulaire d’autorisation signé pour soumettre votre déclaration.\n')
